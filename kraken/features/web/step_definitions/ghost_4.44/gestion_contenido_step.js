...
When("I click on post link", async function () {
  await this.driver.url("http://test.denkitronik.com:3002/ghost/#/posts");
});
...
When("I display the publish options", async function () {
  let publishOptions = await this.driver.$(
    "(//div[@class='gh-publishmenu ember-view'])"
  );
...
When("I click in the schedule button", async function () {
  let publishButton = await this.driver.$(
    "//button[contains(span/text(),'Schedule')]"
  );
  return await publishButton.click();
});
...
When("I click in the publish button", async function () {
  let publishButton = await this.driver.$(
    "//button[contains(span/text(),'Publish')]"
  );
  return await publishButton.click();
});
...
When("I click in the update button", async function () {
  let publishButton = await this.driver.$(
    "//button[contains(span/text(),'Update')]"
  );
  return await publishButton.click();
});

When("I click on a post", async function () {
  let post = await this.driver.$(
    "(//li[@class='gh-list-row gh-posts-list-item'])[last()-1]"
  );
